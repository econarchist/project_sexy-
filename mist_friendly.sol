...
contract Advanced is owned, TokenERC20 {
...
contract UsingFunctions{
    ...
    function tokenFlow(
        ...
        returns (uint) 
    {
        return stringToUint(tokenPrice)*volume/stringToUint(assetPrice);
    }
}
...
contract Econarchy is Advanced, UsingFunctions{
  ...
  //  mapping(bool => mapping(string => Position)) directions; //True for Long positions, False for shorts(this is to save memory, OK ????)
    ...
    event EnterContract(address _player, string _ticker, string _direction, string _volume, uint _time);
    event PartialExecution(address _player, string _ticker, string _direction, string _volume, uint _time);
    event FullExecution(address _player, string _ticker, string _direction, uint _time);
    
    //Constructor
    function Econarchy(uint256 initialSupply)
    Advanced(initialSupply, "Econarchy", "khi") public {}
    ...
    function OpenLong(
        ...
        EnterContract(msg.sender, ticker, "BUY", volume, now);   //Put this event on the blockchain
    }
    ...
    function CloseLong(
        ...
        if (accounts[msg.sender][true][ticker].volume == volumeUint){   //If executed in full ammount
            ...
            FullExecution(msg.sender, ticker, "BUY", now); //Put this event on the blockchain
            ...
            PartialExecution(msg.sender, ticker, "BUY", volume, now); //put `PartialExecution` event on the blockchain
        }
        
        balanceOf[msg.sender] += tokenFlows; //Player gets returns to the balance
        //Adjust total supply 
        if(tokenFlows > executedBetTokens){
            totalSupply+=(tokenFlows - executedBetTokens);   
        }else{
            totalSupply-=(executedBetTokens - tokenFlows);
        }
